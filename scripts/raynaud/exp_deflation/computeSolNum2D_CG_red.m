% Contributors: Timothée Raynaud, Axel Modave

function [solA, sysA] = computeSolNum2D_CG_red(mesh, dofm, PREC)
...
sysA.matM = matM;
sysA.matMevpb = matM;
sysA.matI_int = eye(size(matM,1));
sysA.matR_int = eye(size(matM,1));
sysA.matAred = matA;
sysA.matMred = matM;
sysA.rhsAred = rhsA;
if(~isempty(dofDIR))
    ...
    matM(dofDIR,:) = 0;
    matM(:,dofDIR) = 0;
    matM(dofDIR,dofDIR) = eye(size(dofDIR,1),size(dofDIR,1));
    sysA.matMevpb(dofDIR,:) = 0;
    sysA.matMevpb(:,dofDIR) = 0;
    sysA.matI_int(dofDIR,dofDIR) = 0;
    sysA.matR_int(dofDIR,:) = [];
    sysA.matAred(dofDIR,:) = [];
    sysA.matAred(:,dofDIR) = [];
    sysA.matMred(dofDIR,:) = [];
    sysA.matMred(:,dofDIR) = [];
