...
% outliers : negative eigenvalues and eigenvalues closer to 0 than 0.3
outliers = find(real(eigenval) < 0 | abs(real(eigenval)) < 0.3);
cluster = find(real(eigenval) > 0 & abs(real(eigenval)) > 0.3);

% distance between the outliers and the cluster
% clusterOutliers = nchoose2(cluster,outliers); % -> doesn't work obviously
% D = abs(eigenval(clusterOutliers(:,1)) - eigenval(clusterOutliers(:,2)));
% distance = min(D);
[eigOutliers, eigCluster] = meshgrid(eigenval(outliers),eigenval(cluster));
concat = cat(2,eigOutliers',eigCluster');
concat = reshape(concat,[],2);
distance = min(abs(concat(:,1)-concat(:,2)));

% radius of the cluster : max of the distance between two eigenvalues of the cluster
clusterPairs = nchoosek(cluster,2);
D = abs(eigenval(clusterPairs(:,1)) - eigenval(clusterPairs(:,2)));
radius = max(D);
...
[~,~,~,it,resVec] = gmres(mat,b,[],tol,maxit);
