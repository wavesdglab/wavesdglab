...
function run(benchmark,degree,PREC,tol,maxit,itout,nbEigVec)
    ...
    folder = "output/mainHistoryResidual_cavity";
    if ~exist(folder, 'dir')
        mkdir(folder);
    end
    ...
    writeField2D(dofm, mesh, xADana, folder+"/solADana.pos", "solADana");
    writeField2D(dofm, mesh, xPADana, folder+"/solPADana.pos", "solPADana");
    writeField2D(dofm, mesh, xADnum, folder+"/solADnum.pos", "solADnum");
    writeField2D(dofm, mesh, xPADnum, folder+"/solPADnum.pos", "solPADnum");
