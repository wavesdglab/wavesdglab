...
% 
%     xM = (V1(1,1)+V2(1,1)+V3(1,1)) / 3;
%     yM = (V1(1,2)+V2(1,2)+V3(1,2)) / 3;
% 
%     if ((xM+M)^2+yM^2>0.05^2)
%         normSol2 = normSol2 ...
%             + (1/(eta*c)) * weights(:)' * (refQ .* conj(refQ)) * detJdxdu ...
%             + (eta/c) * weights(:)' * (refVxQ .* conj(refVxQ) + refVyQ .* conj(refVyQ)) * detJdxdu;
%         normErr2 = normErr2 ...
...
for tri=1:mesh.numTri
    ...
    if(~isempty(xSou) && ~isempty(ySou))
        ver = mesh.mapTriToVer(tri,:);
        VX = mesh.coord(ver,1);
        VY = mesh.coord(ver,2);
        dist = sqrt((VX-xSou).^2 + (VY-ySou).^2);
        if (min(dist) < 2*h)
            continue;
        end
    end
    ...
    normSol2 = normSol2 ...
        + (1/(rho*c*c)) * weights(:)' * (refQ .* conj(refQ)) * detJdxdu ...
        + rho * weights(:)' * (refVxQ .* conj(refVxQ) + refVyQ .* conj(refVyQ)) * detJdxdu;
    normErr2 = normErr2 ...
        + (1/(rho*c*c)) * weights(:)' * (errQ .* conj(errQ)) * detJdxdu ...
        + rho * weights(:)' * (errVxQ .* conj(errVxQ) + errVyQ .* conj(errVyQ)) * detJdxdu;
end
