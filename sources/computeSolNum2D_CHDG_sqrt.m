...
function [solI, sysA] = computeSolNum2D_CHDG_sqrt(mesh, dofm, tau, ~, PREC, order)
...
for tri=1:mesh.numTri
    ...
    for fac = 1:3
        ...
        matK_FFel = shapeFluDsQ' * (weightsLinQ .* shapeAuxDsQ) / (Jdxdu); 
...
%         matHFel = 2 * matM_HFel;
        ...
        % Elemental matrices (interface condition)
        matFFel = sqrt(1 + 1/k^2.*matK_FFel);          
