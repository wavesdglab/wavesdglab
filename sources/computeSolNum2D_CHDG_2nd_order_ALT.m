...
function [solI, sysA] = computeSolNum2D_CHDG_2nd_order_ALT(mesh, dofm, tau, ~, PREC)
...
for tri=1:mesh.numTri
    ...
    for fac = 1:3
        ...
        matK_FFel = shapeFluDsQ' * (weightsLinQ .* shapeFluDsQ) / (Jdxdu);       % NEW
        ...
        if (triNeigh > 0)
            ...
            switch BC
                case 'DIR'
                    matGHel = matM_GHel;
                    matGFel = 1/k^2 * beta * matK_GFel;
                    rhsGel  = +2*tau*rhsPel;
                case 'NEU'
                    matGHel = -matM_GHel;
                    rhsGel  = -2*(nx*rhsUel + ny*rhsVel);
                case 'ABC'
                    matGFel = 0.5/k^2 * beta * matK_GFel;
        ...
        % Elemental matrices (interface condition)
        matFFel = matM_FFel - 0.5/(k^2) * beta * matK_FFel;                                      % UPDATED
...
matFH = sparse(matFGx, matFGy, matFHv, numDofFAC, numDofFAC);
...
% Matrix partition
X = (matFF-matFH*(matHH\matHF))\(matFH*(matHH\matHI));
Y = (matFF-matFH*(matHH\matHF))\(matFH*(matHH\matHG)-matFG);
XX = -matHH\(matHI+matHF*X);
YY = -matHH\(matHG+matHF*Y);

sysA.matII = matII + matIH * XX + matIF * X;
sysA.matIG = matIG + matIH * YY + matIF * Y;
sysA.matGI = matGH * XX + matGF * X;
sysA.matGG = matGH * YY + matGF * Y + matGG;
