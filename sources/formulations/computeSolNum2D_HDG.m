...
for tri=1:mesh.numTri
    ...
    for fac = 1:3
        ...
        if (triNeigh > 0)
            ...
            rhsPel = transpose(shapeAuxQ) * (weightsLinQ .* souQ) * Jdxdu;
            rhsUel = transpose(shapeAuxQ) * (weightsLinQ .* souUxQ) * Jdxdu;
            rhsVel = transpose(shapeAuxQ) * (weightsLinQ .* souUyQ) * Jdxdu;
